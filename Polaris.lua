-- all credits to the polaris team: https://discord.gg/dyW8dhpJVW
...
local hurttime = 0
local PolarisRelease = "2.09 Beta"
...
local colors = {
    ...
    ["StopRideWave"] = { -- Stav fav frfrfrfrfrfrfrfr :D
		["Main"] = Color3.fromRGB(124, 7, 191),
		["Second"] = Color3.fromRGB(66, 2, 227)
	},
}
...
local currentTheme = colors.Hackerman
local arrayItems = {}
local arraylist = {
	Create = function(o)
...
FunctionTable.getRemote = function(name)
  local remote
...
local getRemote = FunctionTable.getRemote
		...
		if callback then
			auraConnection = RunService.Heartbeat:Connect(function()
				...
				if nearest ~= nil then
					...
					local nearestPrimaryPartPosition = nearestCharacter.PrimaryPart.Position + Vector3.new(0, 2, 0)
		...
		if callback then
			repeat
				...
				if SpeedMode.Option == "Bedwars" then
					speed = Character:GetAttribute("SpeedBoost") and 0.12 or 0.023
...
Spider = Motion.NewButton({
	...
	Function = function(callback)
		if callback then
			repeat
				local raycastxd = utils.NewRaycast(PrimaryPart.Position,Humanoid.MoveDirection * 2)
				local velo = PrimaryPart.Velocity

				if raycastxd and UserInputService:IsKeyDown("W") or then
		...
		if callback then
			task.spawn(function()
				repeat task.wait()
                    ...
                    if hasItem("orange") then
                        ConsumeRemote:InvokeServer({
							["item"] = inventory:WaitForChild("orange")
						})
                    end
...
AutoDip = Misc.NewButton({
	...
	Function = function(callback)
		if callback then
			...
			repeat task.wait()
				local nearest = getNearestPlayer(AccountForBows.Enabled and 9e9 or 19)
		...
		if callback then
			repeat
				...
				if Sword.Enabled then
					game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("BedwarsPurchaseItem"):InvokeServer({
						[1] = {
							shopItem = {
					...
					game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("BedwarsPurchaseItem"):InvokeServer({
						...
						[2] = {
							shopItem = {
								...
								superiorItems = {"diamond_chestplate"},
								currency = "iron",
								amount = 1,
								category = "Combat",
								ignoredByKit = {"bigman", "tinker"},
						...
						[3] = {
							shopItem = {
								...
								superiorItems = {"emerald_chestplate"},
								currency = "emerald",
								amount = 1,
								category = "Combat",
								ignoredByKit = {"bigman", "tinker"},
				...
				if EraUpgrade.Enabled then
					game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("RequestPurchaseEra"):InvokeServer({
						[1] = {era = "iron_era"},
						[2] = {era = "diamond_era"},
						[3] = {era = "emerald_era"}
					})
...
Sword = AutoBuy.NewToggle({
	Name = "AutoBuySword",
	Function = function() end
})
Armor = AutoBuy.NewToggle({
	Name = "AutoBuyArmor",
	Function = function() end
})
EraUpgrade = AutoBuy.NewToggle({
	Name = "AutoEraUpgrade",
	Function = function() end
})]]

AnticheatBypass = Exploit.NewButton({
	Name = "AnticheatBypass",
	Function = function(callback)
		if callback then
            repeat task.wait()
                game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.TridentUnanchor:InvokeServer()
                game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.TridentUnanchor:FireServer()
            until (not AnticheatBypass.Enabled)
		end
	end,
})
